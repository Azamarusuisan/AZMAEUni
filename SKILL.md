...
- `article.md` uses descriptive Markdown anchors, real Markdown lists, fenced code, and block quotes; exposed raw URLs, URL-only anchor text, pseudo-list markers, and collapsed code-like paragraphs fail preflight;
...
Read the adapter for the requested target: [references/cms-note.md](references/cms-note.md) for note, or [references/cms-brain.md](references/cms-brain.md) for Brain. For note, also read [references/rich-media.md](references/rich-media.md); its semantic text rules apply even when the article has no embeds. Default to note unless the user asks for Brain. Then read exactly one provider reference:
...
5. Apply title, body, headings, lists, quotations, code blocks, and links with the editor's current semantic controls. Never leave Markdown markers, pseudo-list characters, collapsed commands/logs, or exposed raw URLs in ordinary paragraphs; source URLs in code may remain inside code blocks. Every normal link needs meaningful visible text and a verified HTTPS target. Keep every source code fence inside one semantic code block even when it contains blank lines; if the editor exits code formatting on an empty line, use an ASCII-space-only line and compare after right-trimming each line. Never use zero-width or full-width invisible separators. Treat each required `article-package.json embeds` item as the card form of its one exact, unindented standalone Markdown fallback anchor: insert one card at that anchor's position through the current `埋め込み` or equivalent semantic control, do not leave a duplicate normal link in note when the preview succeeds, and verify provider, title, canonical URL, and surrounding block order before continuing. Stop at the first failed card and do not attempt later embeds in that run; this keeps `verified_embeds` an idempotent prefix. Prefer this native card for a planned previous/related note article; keep ordinary inline citations as anchors. Upload each required body image one at a time and observe that it appears at the planned position before sending the next file. Upload the thumbnail separately and observe its preview. Then apply inline hashtags.
6. Save as draft. Never select a public/scheduled state.
7. Observe the saved indicator, reopen or safely reread the same draft, and compare title, heading levels/order, list kinds/items/order, quotes, code blocks, descriptive anchor text/URLs, required embeds, images, thumbnail, hashtags, and a content fingerprint. Expected normal anchors are the source anchors minus successfully rendered embed fallbacks; exclude each matched fallback/card block on both sides of the normalized text fingerprint. Confirm that exposed raw URLs and pseudo-list/code paragraphs are absent. For code, compare the normalized full contents and order of every fence, not only the block count, and confirm that no code-fence text leaked into ordinary paragraphs or lists.
8. Write schema 3 `cms-receipt.json`, including the `rich_text` verification flags, exact ordered `verified_embeds`, expected and observed body-image counts, the ordered run-relative paths actually verified in the editor, and the verified thumbnail path. Use `verified` only when every required item matches; otherwise list each missing `embed:<canonical-url>` or image path and use `save_unverified`.
