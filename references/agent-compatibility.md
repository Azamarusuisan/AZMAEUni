...
```bash
python3 scripts/manage.py doctor --agent <codex|claude|hermes> \
  --browser <chrome|safari> --target <note|brain>
```
...
controls by role, accessible name, label, placeholder, or surrounding heading.
Documentation or a successful note check is not evidence that Brain passed.
